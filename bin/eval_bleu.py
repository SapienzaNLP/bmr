...
def raw_corpus_bleu(hypothesis: Iterable[str], reference: Iterable[str], offset: Optional[float] = 0.01) -> float:
    bleu = sacrebleu.corpus_bleu(
        hypothesis, reference, smooth_value=offset, force=True, use_effective_order=False, lowercase=True
    )
    return bleu.score


def raw_corpus_chrf(hypotheses: Iterable[str], references: Iterable[str]) -> float:
    return sacrebleu.corpus_chrf(hypotheses, references, beta=2, remove_whitespace=True)
