...
from mspring_bmr.evaluation import write_predictions
from mspring_bmr.tokenization_bart import AMRBartTokenizer
import json
from pathlib import Path
from mspring_bmr.IO import read_raw_amr_data
from mspring_bmr.entities import read_entities
...
if __name__ == "__main__":
    ...
    for sent, (i, with_wikis, name_to_entity, name_to_ops) in zip(
        sentences, read_entities(sentences, graphs, just_tagged=not args.all)
    ):
    ...
    (
        ...
        predictions,
        scores,
    ) = main_dense.run(args_blink, logger, *models, test_data=for_blink, device=args.device)
