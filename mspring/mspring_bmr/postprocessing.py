...
from mspring_bmr.linearization import AMRTokens
...
# 
        ...
        elif subtok.startswith(f'{tokenizer.INIT}</lit>'):
            is_lit = False
            tokens.append(subtok.lstrip(tokenizer.INIT))
            backreferences.append(-1)
            current_token_i += 1
...
# 
        ...
        elif not (tokens[-1].startswith(':') or is_lit) and  (subtok == f"{tokenizer.INIT}," or subtok == f"{tokenizer.INIT}."):
            tokens[-1] = tokens[-1] + subtok.lstrip(tokenizer.INIT)

        elif is_lit and tokens[-1] != "<lit>" and  subtok == f"{tokenizer.INIT}:":
            tokens[-1] = tokens[-1] + subtok.lstrip(tokenizer.INIT)

        elif not (tokens[-1].startswith(':') or is_lit) and subtok.lstrip(tokenizer.INIT).isnumeric():
            tokens[-1] = tokens[-1] + subtok.lstrip(tokenizer.INIT)
                
        elif tokens[-1].startswith('http') and (subtok.lstrip(tokenizer.INIT) == ":" or subtok.lstrip(tokenizer.INIT) == ""):
            tokens[-1] = tokens[-1] + subtok.lstrip(tokenizer.INIT)
        ...
        elif (tokens[-1] == ':') and subtok == f"{tokenizer.INIT}s":
            tokens[-1] = tokens[-1] + subtok.lstrip(tokenizer.INIT)

        elif (tokens[-1] == ':op' or tokens[-1] == ':snt') and subtok.lstrip(tokenizer.INIT).isnumeric():
            tokens[-1] = tokens[-1] + subtok.lstrip(tokenizer.INIT)

        elif (tokens[-1].startswith(':op') or tokens[-1].startswith(':snt')) and subtok.endswith('of'):
            tokens[-1] = tokens[-1] + subtok.lstrip(tokenizer.INIT)

        # tokenize special relations
        elif ((tokens[-1].startswith(':prep')) or (tokens[-1].startswith(':conj'))) and not (subtok.startswith(tokenizer.INIT + '(') or subtok.startswith(tokenizer.INIT + '<')):
            tokens[-1] = tokens[-1] + subtok.lstrip(tokenizer.INIT)

        # tokenize multiwords nodes
        elif len(tokens) > 2 and 'pointer' in tokens[-2] and not tokens[-1].startswith(':') and subtok.startswith(tokenizer.INIT) and not (subtok.endswith(')') or subtok.startswith(tokenizer.INIT + ':')):
            tokens[-1] = tokens[-1] + subtok.lstrip(tokenizer.INIT)

        # # if the previous token ends in _ attach to the previous
        elif subtok.startswith(tokenizer.INIT) and (tokens[-1].endswith("_") or (tokens[-1].endswith("-") and tokens[-1] != "-")):
            tokens[-1] = tokens[-1] + subtok.lstrip(tokenizer.INIT)

        elif subtok == f"{tokenizer.INIT}-" and not (tokens[-1].startswith(":") or tokens[-1] == "<lit>" ):
            tokens[-1] = tokens[-1] + subtok.lstrip(tokenizer.INIT)
