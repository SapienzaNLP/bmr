...
def predict_amrs(loader, model, tokenizer, beam_size=1, decoder_start_token_id=0, tokens=None, restore_name_ops=False, return_all=False):
    ...
    if tokens is None:
                ...
                with torch.no_grad():
                    out_dict = model.generate(
                        **x,
                        ...
                        forced_bos_token_id=tokenizer.convert_tokens_to_ids(f"{tokenizer.INIT}("),
        ...
        # scores = [t for tt in scores for t in tt]
    ...
    for i1 in range(0, len(tokens), beam_size):
        ...
        for i2 in range(i1, i1+beam_size):
            ...
            graph.tokens = tokk
            # graph.score = scores[i2].cpu().numpy()
...
def predict_amrs_beam(loader, model, tokenizer, beam_size=1, decoder_start_token_id=0, tokens=None, restore_name_ops=False, return_all=False):
    ...
    if tokens is None:
        ...
        with tqdm(total=total) as bar:
            for x, y, extra in loader:
                ...
                ids.extend(ii)
                with torch.no_grad():
                    encoder_input_ids = x['input_ids']

                    input_ids = torch.ones((beam_size*len(encoder_input_ids), 1), device=model.device, dtype=torch.long)
                    input_ids = input_ids * model.config.decoder_start_token_id

                    model_kwargs = {
                        "encoder_outputs": model.get_encoder()(
                            encoder_input_ids.repeat_interleave(beam_size, dim=0), return_dict=True
                        )
                    }
                    ...
                    beanOut = model.beam_search(input_ids, beam_scorer, logits_processor=logits_processor,  
                        num_beams=beam_size,
                        forced_bos_token_id=tokenizer.convert_tokens_to_ids(f"{tokenizer.INIT}("),
                        decoder_start_token_id=decoder_start_token_id,
                ...
                out = beanOut.sequences
                nseq = len(ii)
                print(out.shape)
                scores.extend(beanOut.sequences_scores)
                tokens.extend(out.cpu().numpy().tolist())
    ...
    graphs = []
    for graph_tok in tokens:
        graph, status, (lin, backr) = tokenizer.decode_amr(graph_tok, restore_name_ops=restore_name_ops)
        graph.status = status
        graph.nodes = lin
        graph.backreferences = backr
        graph.tokens = graph_tok
        graphs.append(graph)

    for gps, gg, score in zip(graphs, loader.dataset.graphs, scores):
        metadata = gg.metadata.copy()
        metadata['annotator'] = 'bart-amr'
        metadata['date'] = str(datetime.datetime.now())
        metadata['score'] = str(score.cpu().numpy())
        if 'save-date' in metadata:
            del metadata['save-date']
...
def compute_smatch_graphs_filtering(graphs, test_path, predictions_path, sorted_pred_path):
    with Path(predictions_path).open() as p, Path(test_path).open() as g:
        graphs_filtered = next(score_amr_pairs_filtering(graphs, p, g))

    pieces = [encode(g) for g in graphs_filtered]
    sorted_pred_path.write_text('\n\n'.join(pieces))
...
def score_amr_pairs_filtering(graphs, f1, f2, justinstance=False, justattribute=False, justrelation=False):
    """
    Score one pair of AMR lines at a time from each file handle
    :param f1: file handle (or any iterable of strings) to read AMR 1 lines from
    :param f2: file handle (or any iterable of strings) to read AMR 2 lines from
    ...
    for (precision, recall, f_score), graph in micro_f_list:
        graph.metadata['scores'] = "Precision: " + str(precision) + " Recall: " + str(recall) + " F-score: " + str(f_score)
        if f_score > 0.8 and f_score < 0.9:
            filtered_graphs.append(graph)

    print(len(micro_f_list))
    print(len(filtered_graphs))

    yield filtered_graphs